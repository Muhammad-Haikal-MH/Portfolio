  ...
  return (
    <div>
      ...
      <motion.div
        ...
        className="dark:bg-[#121212] dark:shadow-neutral-700 bg-white rounded-tr-lg rounded-br-lg text-gray shadow-xl z-[999] w-[16rem] max-w-[16rem]
      ...
      >
        <div className={`${isOpen ? "mx-3 py-12 px-2" : "py-10 px-2"}`}>
          <img
            src={foto}
            alt=".."
            className={`rounded-full object-cover border duration-200 hover:scale-115 ${isOpen ? "h-20 w-20" : "h-12 w-12"}`}
          />
          <div
            className={`flex items-center gap-2 mt-4 ${isOpen ? "block" : "hidden"}`}
          >
          ...
          {/* darkmode */}
          <div
            className={`flex items-center ${isOpen ? "justify-between" : "justify-center"} mt-7`}
          >
            {isOpen && (
              <div className="p-1.5 border-1 border-gray-300 dark:border-gray-700 rounded-2xl text-sm">
                <span className="text-green-500 size-6 animate-pulse mr-1">
                  &bull;
                </span>{" "}
                hire me.
              </div>
            ...
            <div
              className="rounded-xl p-2 flex cursor-pointer items-center justify-center bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 hover:dark:bg-gray-500 active:bg-gray-400 active:dark:bg-gray-600 active:transition duration-500 "
              onClick={handleMode}
            >
              {mode ? (
        ...
        <div className="flex flex-col h-full">
          <ul
            className={`whitespace-pre py-5 flex flex-col gap-3 font-medium overflow-x-hidden ${isOpen ? "px-5" : "px-2.5"}`}
          >
            <li className="group hover:bg-neutral-200 hover:dark:bg-neutral-800 hover:rounded-md hover:scale-110 duration-150">
              <NavLink to="/" className={"link"}>
                <HiOutlineHomeModern
                  size={23}
                  className="min-w-max group-hover:-rotate-[25deg] duration-300"
                />
                Home
              </NavLink>
            </li>
          ...
          <span className={`mt-5 mx-5 text-sm ${isOpen ? "block" : "hidden"}`}>
            COPYRIGHT &copy; 2025 <br /> Muhammad Haikal
          </span>
        </div>
        ...
        <motion.div
          ...
          className="absolute w-fit h-fit z-50 bottom-20 right-2 cursor-pointer"
        >
